...
begin
  ...
  comandoSelect.ParamByName('cd_produto').AsInteger :=
  ...
  begin
    edtProdutoGrupoTributacaoPISCOFINS.Text :=
      comandoSelect.FieldByName('cd_tributacao_pis_cofins').Value;
  end;
  edtProdutoNomeGrupoTributacaoPISCOFINS.Text :=
    comandoSelect.FieldByName('nm_tributacao_pis_cofins').AsString;
  ...
  begin
    imagem.Picture.LoadFromFile(GetCurrentDir + '\imagens_produto\' +
      comandoSelect.FieldByName('imagem').AsString);
  end;
...
begin
  ...
  sqltributacao.Close;
  sqltributacao.SQL.Text := 'select ' + 'cd_tributacao, ' +
    'nm_tributacao_icms ' + 'from ' + 'grupo_tributacao_icms ' + 'where ' +
    'cd_tributacao = :cd_tributacao';
  sqltributacao.ParamByName('cd_tributacao').AsInteger :=
    StrToInt(edtProdutoGrupoTributacaoICMS.Text);
  sqltributacao.Open();
  edtProdutoNomeGrupoTributacaoICMS.Text := sqltributacao.FieldByName
    ('nm_tributacao_icms').AsString;
end;
...
begin
  ...
  sqltributacao.Close;
  sqltributacao.SQL.Text := 'select ' + 'cd_tributacao, ' + 'nm_tributacao_ipi '
    + 'from ' + 'grupo_tributacao_ipi ' + 'where ' +
    'cd_tributacao = :cd_tributacao';
  sqltributacao.ParamByName('cd_tributacao').AsInteger :=
    StrToInt(edtProdutoGrupoTributacaoIPI.Text);
  sqltributacao.Open();
  edtProdutoNomeGrupoTributacaoIPI.Text := sqltributacao.FieldByName
    ('nm_tributacao_ipi').AsString;
end;
...
begin
  ...
  sqltributacao.Close;
  sqltributacao.SQL.Text := 'select ' + 'cd_tributacao, ' +
    'nm_tributacao_pis_cofins ' + 'from ' + 'grupo_tributacao_pis_cofins ' +
    'where ' + 'cd_tributacao = :cd_tributacao';
  sqltributacao.ParamByName('cd_tributacao').AsInteger :=
    StrToInt(edtProdutoGrupoTributacaoPISCOFINS.Text);
  sqltributacao.Open();
  edtProdutoNomeGrupoTributacaoPISCOFINS.Text :=
    sqltributacao.FieldByName('nm_tributacao_pis_cofins').AsString;
end;

procedure TfrmCadProduto.excluir;
begin
  if (Application.MessageBox('Deseja Excluir o Produto?', 'Atenção', MB_YESNO)
    = IDYES) then
  begin
    comandosql.Close;
    ...
    comandosql.ParamByName('cd_produto').AsInteger :=
      StrToInt(edtPRODUTOCD_PRODUTO.Text);
    ...
    try
      ...
      on E: exception do
      begin
        ShowMessage('Erro ao excluir o produto ' + edtPRODUTOCD_PRODUTO.Text +
          E.Message);
      end;
    end;
  end
...
begin
  ...
  begin
    if (Application.MessageBox('Deseja Fechar?', 'Atenção', MB_YESNO) = IDYES)
    then
    begin
      Close;
    end;
...
begin
  ...
  if Button = mbRight then
    if (Application.MessageBox('Deseja Excluir a Imagem do Produto?', 'Aviso',
      MB_YESNO) = IDYES) then
    begin
      imagem.Picture := nil;
      conexao.ExecSQL
        ('update produto set imagem = NULL where cd_produto = :cd_produto',
        [StrToInt(edtPRODUTOCD_PRODUTO.Text)]);
    end
    else
    begin
...
const
  SQL = 'select  ' + '    * ' + 'from ' + '    produto_cod_barras ' + 'where ' +
...
begin
  ...
  dm.queryCodBarraProduto.ParamByName('cd_produto').AsInteger :=
    StrToInt(edtPRODUTOCD_PRODUTO.Text);
...
begin
  ...
  sqlVerificaTributacao.ParamByName('cd_produto').AsInteger :=
    StrToInt(edtPRODUTOCD_PRODUTO.Text);
  ...
  begin
    ...
    comandosql.ParamByName('fator_conversao').AsCurrency :=
      StrToCurr(edtPRODUTOFATOR_CONVERSAO.Text);
    comandosql.ParamByName('peso_liquido').AsCurrency :=
      StrToCurr(edtPRODUTOPESO_LIQUIDO.Text);
    comandosql.ParamByName('peso_bruto').AsCurrency :=
      StrToCurr(edtPRODUTOPESO_BRUTO.Text);
    comandosql.ParamByName('observacao').AsString := memoObservacao.Text;
    comandosql.ParamByName('imagem').AsString := NomeImg;

    sqltributacao.Close;
    sqltributacao.SQL.Text :=
      'update                                                     ' +
      'produto_tributacao                                     ' +
      'set                                                       ' +
      'cd_produto = :cd_produto,                            ' +
      ...
      'where                                   ' + 'cd_produto = :cd_produto';
    sqltributacao.ParamByName('cd_produto').AsInteger :=
      StrToInt(edtPRODUTOCD_PRODUTO.Text);
    sqltributacao.ParamByName('cd_tributacao_icms').AsInteger :=
      StrToInt(edtProdutoGrupoTributacaoICMS.Text);
    sqltributacao.ParamByName('cd_tributacao_ipi').AsInteger :=
      StrToInt(edtProdutoGrupoTributacaoIPI.Text);
    sqltributacao.ParamByName('cd_tributacao_pis_cofins').AsInteger :=
      StrToInt(edtProdutoGrupoTributacaoPISCOFINS.Text);
  ...
  begin
    ...
    comandosql.SQL.Text := 'insert into               ' +
      'produto (cd_produto, ' + 'fl_ativo,            ' +
      'desc_produto,        ' + 'un_medida,           ' +
      'fator_conversao,     ' + 'peso_liquido,        ' +
      'peso_bruto,          ' + 'observacao,          ' +
      'imagem)              ' + ' values (:cd_produto,         ' +
      ':fl_ativo,           ' + ':desc_produto,       ' +
      ':un_medida,          ' + ':fator_conversao,    ' +
    ...
    sqltributacao.SQL.Text := 'insert into                      ' +
      'produto_tributacao(cd_produto, ' + 'cd_tributacao_icms,            ' +
      'cd_tributacao_ipi,             ' + 'cd_tributacao_pis_cofins)      ' +
      'values (:cd_produto,                   ' +
      ':cd_tributacao_icms,           ' + ':cd_tributacao_ipi,            ' +
...
begin
  if (edtPRODUTOCD_PRODUTO.Text = EmptyStr) or
    (edtPRODUTODESCRICAO.Text = EmptyStr) or
    (edtPRODUTOUN_MEDIDA.Text = EmptyStr) then
  begin
    raise exception.Create
      ('Código, Descrição e Unidade de Medida não podem ser vazios');
  end
  else if (edtProdutoGrupoTributacaoICMS.Text = EmptyStr) or
    (edtProdutoGrupoTributacaoIPI.Text = EmptyStr) or
    (edtProdutoGrupoTributacaoPISCOFINS.Text = EmptyStr) then
  begin
...
const
  SQL = 'select ' + '   un_medida ' + 'from ' + '   produto_cod_barras ' +
    'where ' + '   cd_produto = :cd_produto';
begin
  if (Trim(edtCodigoBarras.Text) = EmptyStr) or
    (Trim(edtUnCodBarras.Text) = EmptyStr) or (cbTipoCodBarras.ItemIndex = -1)
  then
  ...
  dm.query.ParamByName('cd_produto').AsInteger :=
    StrToInt(edtPRODUTOCD_PRODUTO.Text);
  dm.query.Open(SQL);

  if dm.query.FieldByName('un_medida').AsString = edtUnCodBarras.Text then
    raise exception.Create
      ('O produto já possui código de barras cadastrado para a unidade de medida informada');
